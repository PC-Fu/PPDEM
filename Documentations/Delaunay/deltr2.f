...
c
      ...
      implicit none
c
      integer nmax, nvmax, nlmax, ndmax
      integer nfmax, nhmax, nkmax, njmax, npmax
      parameter (nmax = 160000, nvmax = 2.1*nmax)
      parameter (nlmax = 10000, ndmax = 30000)
      parameter (nfmax = 5 * 400, nhmax = 5 * 400)
      parameter (nkmax = ndmax+1, njmax = 2 * nkmax, npmax = 10000)
c
...
c
      character*30 vninf, vsinf, voinf, lsinf, dlinf,
     *             tninf, toinf, ccinf, npinf, trinf
...
c  30 format(a1)
   50 format(a30)
      ...
      vnflg=.true.
      if(vnflg) then
c        write(*,*)'Delaunay triangulation will be computed from ',
c    *             'scratch.'
c        write(*,*)' '
         write(*,*)'Specify name of file with points ',
     *             '(x-, y-coordinates) for which'
         write(*,*)'triangulation is to be computed:'
         read(5,50)vninf
      endif

      write(*,*)' '
      write(*,*)'Enter icsfig, i. e. number of significant figures ',
     *          'of decimal part'
      write(*,*)'of point coordinates, -1 < icsfig < 10 (total ',
     ...
     *          'either the left or'
      ...
      toflg=.true.
      if(toflg) then
         write(*,*)'Specify name of output triangulation file:'
         read(5,50) toinf
      endif
...
c
      write(*,*)' '
      write(*,*)'Reading points file (x-,y-coordinates) ... '

      open(unit=11,file=vninf,status='old')
      nv = 0
  100 continue
      read(11,*,end=110) xcor, ycor
      nv = nv + 1
  ...
  110 continue
      ...
      if(nv.lt.3) stop 20
      write(*,*)' '
      write(*,*)'Number of points read =',nv
      n=nv
      iase=1
      iaze(1)=nv+1
...
c
      write(*,*)' '
      write(*,*)'Transforming input point coordinates into ',
...
c
      write(*,*)' '
      write(*,*)'Number of points read that were duplications =',idupl
c     write(*,*)'Number of points read that were not processed =',inpro
c
c**********************************************************************
c
...
c
      write(*,*)' '
      write(*,*)'Saving information about final Delaunay ',
     *          'triangulation ... '

      open(unit=16,file=toinf,status='new')
      write(16,*) nv, ivnxt, nvmz, icsfig
      write(16,*)((icon(i,j),i=1,6),j=1,ivnxt)
      write(16,*)(is(i),i=1,nv)
      close(unit=16)
...
c
      write(*,*)' '
      write(*,*)'Testing consistency of final triangulation ... '
      call contst(icon, is, id, nv, ivnxt, nvmz)
c
c*********************************************************************
c
c     test circle criterion
c
      write(*,*)' '
      write(*,*)'Testing circle criterion ... '
      call circri(x, y, ix, iy, ix2, iy2, icon, mhalf, mfull,
     *            isclp, ivnxt, epf)
c
c*********************************************************************
c
c     test orientation of triangles
c
      write(*,*)' '
      write(*,*)'Testing orientation of triangles ... '
      call orient(x, y, ix, iy, ix2, iy2, icon, ivnxt, mhalf, mfull,
     *            isclp, epf)
c
c*********************************************************************
c
c     test convexity of union of triangles
c
      write(*,*)' '
      write(*,*)'Testing convexity of union of triangles ... '
      call convex(x, y, ix, iy, ix2, iy2, icon, is, nv, ivnxt,
     *             nvmz, mhalf, mfull, isclp, epf)
c
c********************************************************************
c
c     computations are completed
c
      write(*,*)' '
      write(*,*)'Computations completed.'
      write(*,*)' '
      write(*,*)'Number of vertices in final triangulation: ',nv
      write(*,*)'Number of triangles: ',ivnxt
      write(*,*)' '
c
