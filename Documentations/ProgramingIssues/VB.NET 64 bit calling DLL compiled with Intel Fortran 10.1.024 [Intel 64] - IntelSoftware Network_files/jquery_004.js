...
$.fn.rating = function(instanceSettings){
 ...
 //  if($.rating.groups[n].readOnly || settings.required){
  ...
  eStar = $('<div class="star" onclick="alertstuff(this);" title="' + (this.title || this.value) + '"><a title="' + (this.title || this.value) + '">' + this.value + '</a></div>');
  $(this).after(eStar);
...
function alertstuff(t){
 if(t.className.indexOf("star_readonly") > 0){
	 alert(t.title);
 }
}
