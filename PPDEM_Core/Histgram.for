	...
	double precision xEle(nEle),yEle(nEle),mInertEle(nEle),qEle(nEle),
     ...
     $   BinFN(nBinNorm),BinFT(nBinNorm),BinRoseFR(nBinNorm),
     $   maxBinF,maxBinE,
     $   binFIntv,binEIntv,binFRIntv,maxBinFR,maxBinNorm,
     $   maxBinRoseFR(nBinNorm),binNormIntv,fctSldNorm
     ...
     $ ,nEleMask,nConMask,temp,temp1,temp2
     ...
     $        xPA,yPA,flagFMask(iCon))
         ...
         nConMask=nConMask+flagFMask(iCon)
     ...
     $                  +max((1-jWeight),0))
                    ...
                    maxBinE=maxBinE+(elong(iEle)-1)*mInertEle(iEle)
     $                  *(min(jWeight,1)*ptcStress(iEle,max(jWeight,1))
     $                  +max((1-jWeight),0))
          ...
          do iCon=1,nCon
             ...
             if (flagFMask(iCon).eq.1) then
                ...
                BinFN(int(qNorm(iCon)/binNormIntv)+1)=
     $              BinFN(int(qNorm(iCon)/binNormIntv)+1)
     $              +abs(FCon(iCon))/sqrt(1+fricRatio(iCon)**2)
     
                BinFT(int(qNorm(iCon)/binNormIntv)+1)=
     $              BinFT(int(qNorm(iCon)/binNormIntv)+1)
     $              +abs(FCon(iCon))/sqrt(1+fricRatio(iCon)**2)
     $              *fricRatio(iCon)
     ...
     $              BinNorm(int(qNorm(iCon)/binNormIntv)+1)+1
                    ...
                    maxBinNorm=maxBinNorm+1
                    
                   BinRoseFR(int(qNorm(iCon)/binNormIntv)+1)=
     $             BinRoseFR(int(qNorm(iCon)/binNormIntv)+1)+
     $             fricRatio(iCon)
                   
                   maxBinRoseFR(int(qNorm(iCon)/binNormIntv)+1)=
     $             maxBinRoseFR(int(qNorm(iCon)/binNormIntv)+1)+
     $             1
     ...
     $              +abs(FCon(iCon))
                   ...
                   maxBinF=maxBinF+abs(FCon(iCon))
         
                   BinNorm(int(qNorm(iCon)/binNormIntv)+1)=
     $              BinNorm(int(qNorm(iCon)/binNormIntv)+1)
     $              +abs(FCon(iCon))
                   
                   maxBinNorm=maxBinNorm+abs(FCon(iCon))
                   
                   BinRoseFR(int(qNorm(iCon)/binNormIntv)+1)=
     $             BinRoseFR(int(qNorm(iCon)/binNormIntv)+1)+
     $             fricRatio(iCon)*abs(FCon(iCon))
     ...
     $              +abs(FCon(iCon))
                   ...
                   maxBinFR=maxBinFR+abs(FCon(iCon))
          ...
          BinOri=BinOri/maxBinE/6.2832*nBinOri
          BinForce=BinForce/maxBinF/6.2832*nBinForce
          BinFR=BinFR/maxBinFR*(nBinFR-1)
            ! BinFR are not used for plotting.  BinFR(nBinFR) will be used to store of the percentage of contacts in sliding.
          BinNorm=BinNorm/maxBinNorm/6.2832*nBinNorm
          BinSlideNorm=BinSlideNorm/maxBinNorm/6.2832*nBinNorm
          
          do jBin=1, nBinNorm
            if (maxBinRoseFR(jBin).gt.0) then
            BinRoseFR(jBin)=BinRoseFR(jBin)/maxBinRoseFR(jBin)
            end if
          end do
          ...
          do jBin=1,nBinForce*2
            ...
            ptBinF(jBin*2-1,1)=BinForce(iBin)*cos(binFIntv*(jBin-1))
            ptBinF(jBin*2,1)=BinForce(iBin)*cos(binFIntv*jBin)
            ptBinF(jBin*2-1,2)=BinForce(iBin)*sin(binFIntv*(jBin-1))
            ptBinF(jBin*2,2)=BinForce(iBin)*sin(binFIntv*jBin)
          ...
          do jBin=1,nBinOri*2
            ...
            if (jBin.gt.nBinOri) then
             ...
             ptBinE(jBin*2-1,1)=BinOri(iBin)*cos(binEIntv*(jBin-1))
             ptBinE(jBin*2,1)=BinOri(iBin)*cos(binEIntv*(jBin))
             ptBinE(jBin*2-1,2)=BinOri(iBin)*sin(binEIntv*(jBin-1))
             ptBinE(jBin*2,2)=BinOri(iBin)*sin(binEIntv*jBin)
     ...
     $              BinNorm(iBin)*cos(binNormIntv*(jBin-1))
             ptBinNorm(jBin*2,1)=BinNorm(iBin)
     $          *cos(binNormIntv*(jBin))
             ptBinNorm(jBin*2-1,2)=
     $              BinNorm(iBin)*sin(binNormIntv*(jBin-1))
             ptBinNorm(jBin*2,2)=BinNorm(iBin)*sin(binNormIntv*jBin)
             
             
             
             ptBinFN(jBin*2-1,1)=
     $              BinFN(iBin)*cos(binNormIntv*(jBin-1))
             ptBinFN(jBin*2,1)=BinFN(iBin)
     $          *cos(binNormIntv*(jBin))
             ptBinFN(jBin*2-1,2)=
     $              BinFN(iBin)*sin(binNormIntv*(jBin-1))
             ptBinFN(jBin*2,2)=BinFN(iBin)*sin(binNormIntv*jBin)

             ptBinFT(jBin*2-1,1)=
     $              BinFT(iBin)*cos(binNormIntv*(jBin-1))
             ptBinFT(jBin*2,1)=BinFT(iBin)
     $          *cos(binNormIntv*(jBin))
             ptBinFT(jBin*2-1,2)=
     $              BinFT(iBin)*sin(binNormIntv*(jBin-1))
             ptBinFT(jBin*2,2)=BinFT(iBin)*sin(binNormIntv*jBin)


             ptBinRoseFR(jBin*2-1,1)=
     $              BinRoseFR(iBin)*cos(binNormIntv*(jBin-1))
             ptBinRoseFR(jBin*2,1)=BinRoseFR(iBin)
     $          *cos(binNormIntv*(jBin))
             ptBinRoseFR(jBin*2-1,2)=
     $              BinRoseFR(iBin)*sin(binNormIntv*(jBin-1))
             ptBinRoseFR(jBin*2,2)=BinRoseFR(iBin)*sin(binNormIntv*jBin)


             ptBinSlideNorm(jBin*2-1,1)=
     $              BinSlideNorm(iBin)*cos(binNormIntv*(jBin-1))
             ptBinSlideNorm(jBin*2,1)=BinSlideNorm(iBin)
     $          *cos(binNormIntv*(jBin))
             ptBinSlideNorm(jBin*2-1,2)=
     $              BinSlideNorm(iBin)*sin(binNormIntv*(jBin-1))
             ptBinSlideNorm(jBin*2,2)=
     $              BinSlideNorm(iBin)*sin(binNormIntv*jBin)
          ...
          do iBin=1,nBinFR
             ...
             ptBinFR(iBin,2)=162-80*BinFR(iBin)
             ptBinFR(iBin,3)=250.0/nBinFR
             ptBinFR(iBin,4)=80*BinFR(iBin)
             
          
          end do
          BinFR(nBinFR)=BinFR(nBinFR)/(nBinFR-1)
          ! BinFR are not used for plotting.  BinFR(nBinFR) will be used to store of the percentage of contacts in sliding.
...
!        flagFMask(iCon)=0
     ...
     $  ( tsStress(i+4),i=1,4)
      ...
      end if    !If ncon=0, do not execute this
