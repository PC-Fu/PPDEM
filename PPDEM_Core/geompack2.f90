...
!    in VCL array.
!
!    Input, integer NPOLG, the number of polygons or positions used 
!    in HVL array.
!
!    Input, integer NVERT, the number of vertices or positions used in 
!    PVL array.
!
...
!  Parameters:
!
!    Input, integer NHOLE, the number of holes in the region.
!
...
!    Input, real ( kind = 8 ) VCL(1:2,1:*), the vertex coordinate list.
!
!    Input, integer HVL(1:NHOLE), the head vertex list; HVL(I) is index in 
...
!    routine; K <= LDA.
!
!    Input, integer A(1:K,1:*), the array of at least N K-dimensional 
!    integer points.
!
...
!
!    Input, integer INDPVL(0:NVRT), indices in PVL of vertices; INDPVL(I) = -K
...
!
!    Output, integer PRIME, the smallest prime greater than or equal to
...
!
  ...
  integer m
  integer prime
  integer, save, dimension ( nprime ) :: primes = (/ &
...
!
!    Input, integer AXIS, integer NPTAV; if AXIS < 1 or K < AXIS, then uniform 
...
!
!    Input/output, integer TIL(3,*), the triangle incidence list.  
...
!    Input, integer HVL(1:NPOLG), the head vertex list.
!
!    Input, integer PVL(1:4,1:NVERT), real ( kind = 8 ) IANG(1:NVERT), the 
